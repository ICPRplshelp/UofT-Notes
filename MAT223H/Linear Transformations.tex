...
\hypersetup{
  pdftitle={MAT223H},
  pdfauthor={https://github.com/ICPRplshelp},
  hidelinks,
...
\makeatother
\subtitle{Linear Transformations onwards}
\author{\url{https://github.com/ICPRplshelp}}
\date{12/15/2021}
...
\textbf{Remark.} To show that a function is linear, you can combine (i)
and (ii) together.
...
\includegraphics[width=1.93395in,height=1.49198in]{TEXIMAGEF_Linear_Transformations/media/image1.png}
...
\begin{align*}
\left\lbrack x_{1} - x_{3},\ x - 2 + x_{3} \right\rbrack &= \lbrack 1,\ 2\rbrack \Rightarrow \begin{matrix}
x_{1} - x_{3} = 1 \\
x_{2} + x_{3} = 2 \\
\end{matrix}
 \\
...
0 \\
0 \\
\end{matrix} \right\rbrack \Rightarrow \lbrack s,\  - s,\ s\rbrack,\ s\mathbb{\in R}
 \\
...
\textbf{Definition.} Let $A$ be an $m \times n$ matrix
...
\alpha_{1}{\mathbf{v}}_{1} + \alpha_{2}{\mathbf{v}}_{2} + \ldots + \alpha_{i}{\mathbf{v}}_{i} + \ldots + \alpha_{k}{\mathbf{v}}_{k} &= 0
 \\
\alpha_{1}{\mathbf{v}}_{1} + \alpha_{2}{\mathbf{v}}_{2} + \ldots + \alpha_{i - 1}{\mathbf{v}}_{i - 1} + \ldots + \alpha_{k}{\mathbf{v}}_{k} &= - \alpha_{i}{\mathbf{v}}_{i}
 \\
...
\includegraphics[width=1.60645in,height=1.15312in]{TEXIMAGEF_Linear_Transformations/media/image2.png}
...
\emph{Proof.} Check if $\mathbf{0}$ belongs to $W$.
...
\begin{align*}
\mathbf{u} + r\mathbf{v} &= \left( \alpha{\mathbf{w}}_{1} + \ldots + \alpha_{k}{\mathbf{w}}_{k} \right) + \left( r\beta_{1}{\mathbf{w}}_{1} + r\beta_{2}{\mathbf{w}}_{2} + \ldots + r\beta_{k}{\mathbf{w}}_{k} \right)
 \\
&= \left( \alpha_{1} + r\beta_{1} \right){\mathbf{w}}_{1} + \left( \alpha_{2} + r\beta_{2} \right){\mathbf{w}}_{2} + \ldots + \left( \alpha_{n} + r\beta_{n} \right){\mathbf{w}}_{k}
\end{align*}
...
\textbf{Theorem.} Let $A \in M_{n \times n}$. The following are
...
\emph{Proof.} $1 \Rightarrow 2$ is the process of solving a linear
...
\textbf{Theorem.} Let $A$ be $m \times n$ matrix. $(m > n)$. The
...
\textbf{Remark.} The number of solutions will depend on whether free
...
\textbf{Theorem.} The rank equation of a matrix, where $H$ is in REF
or RREF:
...
&= x_{1}\begin{bmatrix}
 \mid \\
T\left( {\mathbf{e}}_{1} \right) \\
 \mid \\
\end{bmatrix} + x_{2}\begin{bmatrix}
 \mid \\
T\left( {\mathbf{e}}_{2} \right) \\
 \mid \\
\end{bmatrix} + \cdots + x_{n}\begin{bmatrix}
 \mid \\
T\left( {\mathbf{e}}_{n} \right) \\
 \mid \\
\end{bmatrix}
\end{align*}
...
\textbf{Theorem.} Preservation of subspaces.
...
\includegraphics[width=3.19435in,height=2.11728in]{TEXIMAGEF_Linear_Transformations/media/image3.png}
...
\includegraphics[width=1.90212in,height=1.65986in]{TEXIMAGEF_Linear_Transformations/media/image4.png}

\textbf{Remark.} Let $T:\mathbb{R}^{n} \rightarrow \mathbb{R}^{m}$ be
...
$\dim\left( \text{col}(A) \right) = 3 \Rightarrow Range(T)$ is
strictly a subset of $R^{4}$ and is NOT $R^{4}$. The linear
transformation is not onto.

\includegraphics[width=2.45062in,height=1.44393in]{TEXIMAGEF_Linear_Transformations/media/image5.png}
...
\includegraphics[width=2.71605in,height=0.84325in]{TEXIMAGEF_Linear_Transformations/media/image6.png}
...
\textbf{Definition.} The determinant of a $1 \times 1$ matrix is just
its entry.
...
\mathbf{a} \cdot \mathbf{v} &= \left( a_{1},\ a_{2},\ a_{3} \right) \cdot \left( \mathbf{a} \times \mathbf{b} \right)
 \\
&= \left( a_{1},\ a_{2},\ a_{3} \right) \cdot \left( \left| \begin{matrix}
a_{2} & a_{3} \\
b_{2} & b_{3} \\
\end{matrix} \right|,\  - \left| \begin{matrix}
a_{1} & a_{3} \\
...
\includegraphics[width=1.14198in,height=0.94156in]{TEXIMAGEF_Linear_Transformations/media/image7.png}
...
\begin{align*}
\text{Area}^{2} &= \left( \left| \left| \mathbf{a} \right| \right|\left| \left| \mathbf{h} \right| \right| \right)^{2} = \left( \left| \left| \mathbf{a} \right| \right| \cdot \left| \left| \mathbf{b} \right| \right| \cdot \sin^{2}\theta \right)  \\
&= \left( \left| \left| \mathbf{a} \right| \right| \cdot \left| \left| \mathbf{b} \right| \right| \cdot (1 - \cos^{2}\theta \right)
\end{align*}


\includegraphics[width=1.80651in,height=1.39506in]{TEXIMAGEF_Linear_Transformations/media/image8.png}
...
\includegraphics[width=1.7554in,height=1.60494in]{TEXIMAGEF_Linear_Transformations/media/image9.png}
...
\includegraphics[width=1.82921in,height=1.7963in]{TEXIMAGEF_Linear_Transformations/media/image10.png}

If all vectors are parallel to each other, they define a 1-dimensional
object.

\includegraphics[width=1.05556in,height=1.14in]{TEXIMAGEF_Linear_Transformations/media/image11.png}
...
\includegraphics[width=1.14815in,height=0.94763in]{TEXIMAGEF_Linear_Transformations/media/image12.png}
...
$\mathbf{v} \neq 0$ is a solution for $(A - I\lambda)$. However,
because $(A - I\lambda)\mathbf{v} = \mathbf{0}$, then
$(A - I\lambda)$'s determinant is zero.
...
\Leftrightarrow E_{\lambda} &= \left\{ \mathbf{x} \in \mathbb{R}^{n}\mid A\mathbf{x} - \lambda\mathbf{x} = 0 \right\}
 \\
&= \left\{ \mathbf{x} \in \mathbb{R}^{n}\mid(A - \lambda I)\mathbf{x} = 0 \right\}
\end{align*}
...
$$= (2 - \lambda)( - 1)^{1 + 1}\left| \begin{matrix}
2 - \lambda & - 1 \\
3 & - 2 - \lambda \\
\end{matrix} \right| = (2 - \lambda)\left( (2 - \lambda)( - 2 - \lambda) + 3 \right) = (2 - \lambda)\left( \lambda^{2} - 1 \right) = (2 - \lambda)(\lambda - 1)(\lambda + 1)$$
...
\begin{align*}
&= \det\left( P^{- 1}AP - P^{- 1} \cdot \lambda I \cdot P \right) = \det\left( P^{- 1}(A - \lambda I)P \right)
 \\
...
\textbf{Definition.} Let $A$ be an $n \times n$ matrix with
...
0 & 0 & 0 \\
0 & 0 & 0 \\
\end{bmatrix};\text{nullity}(A - 6I) = 2
\end{align*}
...
&\dim\left( E_{\lambda = 1} \right) = 1
 \\
&E_{\lambda = 1} = sp\left( \begin{bmatrix}
1 \\
1 \\
1 \\
\end{bmatrix} \right)
...
A &= PDP^{- 1}
 \\
\Rightarrow A^{k} &= \left( PDP^{- 1} \right)\left( PDP^{- 1} \right)\left( PDP^{- 1} \right)\cdots\left( PDP^{- 1} \right)\text{\ k\ }\text{times}
 \\
...
Also, if $A$ is upper-triangular the eigenvalues are the diagonals of
the matrix.
